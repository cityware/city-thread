...
namespace Cityware\Thread;

declare(ticks = 1);

/**
 * Threading is an abstraction class for PHP's posix and pcntl extensions.
 * 
...
class Threading {
     ...
     * 
     * This setting has an impact on performance for Threading so play
     ...
     * 
     * Returns an instanced Threading object or dies on failure
     * 
     ...
     * 
     * @return object	Instanced Threading object
     */
    public function __construct($maxThreads = 10, $threadTimeLimit = 300, $daemon = false, $logTo = false, $debugLogging = false) {
        if (function_exists('pcntl_fork') && function_exists('posix_getpid')) {
            ...
            die("Threading requires both the POSIX and PCNTL extensions to operate.\n");
        }
    }
    ...
    /**
     * Registers a callback function for the signal dispatcher or for special signals used by Threading
     * 
     * Special signals are:
    ...
    /**
     * Initiates the shutdown procedure for Threading
     * 
